...
from intertext_graph.itsentsplitter import (
    IntertextSentenceSplitter,
    SentenceSplitter,
    make_sentence_nodes,
)
...
class TEIXMLParser(IntertextParser):
        ...
        for child in section:
            ...
            if child.tag == f"{prefix}p":
                ...
                (
                    paragraph_text,
                    children_ix,
                ) = TEIXMLParser._flatten_xml_element_with_child_ix(paragraph)
                TEIXMLParser._check_text(paragraph_text, "paragraph")

                # get references in text
                reference_ixs = [
                    (t, pre_ix, pre_ix + contl)
                    for (t, pre_ix, contl) in children_ix
                    if t.tag == f"{prefix}ref"
                ]

                # ACL layout specific --> lists start with bulletpoints
                if paragraph_text[0] == "\u2022":
                    if list_parent is None:
                        list_parent = TEIXMLParser._add_node(
                            sub_graph,
                            "",
                            NTYPE_LIST,
                ...
                fxid, figure = TEIXMLParser._parse_figure(
                    sub_graph, child, prefix, predecessor, root
                )
                predecessor = figure
                figures[fxid] = figure
                ...
                # add the rest
                logging.info(
                    f"UNKNOWN TAG {section.tag} within section -- adding as flattend version."
                )
                (
                    paragraph_text,
                    children_ix,
                ) = TEIXMLParser._flatten_xml_element_with_child_ix(child)
                TEIXMLParser._check_text(paragraph_text, "paragraph")
    ...
    def _parse_abstract(self, doc, abstract, prefix, predecessor, article_title):
        ...
        for child in abstract:
            ...
            if child.tag != f"{prefix}p":
                ...
                logging.info(
                    f"Unexpected paragraph tag '{child.tag}' in the abstract! Treating as in-line text."
                )
    ...
    def _parse_figure(doc, figure, prefix, predecessor, parent):
        ...
        if "type" in figure.attrib and figure.attrib["type"] == "table":
            ...
            table_node = TEIXMLParser._add_node(
                doc,
            ...
            table_content = TEIXMLParser._add_node(
                doc,
            ...
            figure_node = TEIXMLParser._add_node(
                doc,
                head.text if head.text is not None else "",
                NTYPE_FIGURE,
                meta={
            ...
            figure_content = TEIXMLParser._add_node(
                doc,
    ...
    def _parse_bibitem(bib_item, prefix):
        ...
        if publishing_info:
            ...
            pub_date = (
                pub_date.attrib["when"]
                if pub_date is not None and "when" in pub_date.attrib
                else None
            )
        else:
            pub_title = None
            ...
            if content is None:
                ...
                if pred >= 0:
                    ...
                    new_mergedix = (
                        ...
                        + [
                            (t, prel + len(pre_content), contl)
                            for (t, prel, contl) in mergedix
                        ]
                    )
    ...
    def _add_node(doc, content, ntype, meta=None, predecessor=None, parent=None):
        new_node = Node(content=content, ntype=ntype, meta=meta)
        doc.add_node(new_node)

        if parent is not None:
            parent_edge = Edge(src_node=parent, tgt_node=new_node, etype=Etype.PARENT)
            doc.add_edge(parent_edge)

        if predecessor is not None:
            next_edge = Edge(src_node=predecessor, tgt_node=new_node, etype=Etype.NEXT)
    ...
    def _add_subtree(doc, subTree, lastSubTree, targetParent, targetPredecessor):
        ...
        for ce in new_pseudo_root.get_edges(
            Etype.PARENT, outgoing=True, incoming=False
        ):
            new_parent = Edge(
                src_node=targetParent, tgt_node=ce.tgt_node, etype=Etype.PARENT
            )
            doc.add_edge(new_parent)
            doc.remove_edge(ce)

        # if no other nodes except root: skip the next part
        new_pseudo_next_edges = new_pseudo_root.get_edges(
            Etype.NEXT, outgoing=True, incoming=False
        )
        ...
        end_subtree = (
            new_nodes[lastSubTree.ix] if len(new_pseudo_next_edges) > 0 else None
        )
    ...
    def _parse_document(self) -> IntertextDocument:
        ...
        itg_doc = IntertextDocument(nodes=[], edges=[], prefix=prefix)
        ...
        title = (
            tree.getroot()
            .find(f"{prefix}teiHeader/{prefix}fileDesc/{prefix}titleStmt/{prefix}title")
            .text
        )
        ...
        # PARSE THE ABSTRACT
        #
        abstract = tree.getroot().find(
            f"{prefix}teiHeader/{prefix}profileDesc/{prefix}abstract/{prefix}div"
        )
        abstract_title_node, predecessor = self._parse_abstract(
            itg_doc, abstract, prefix, predecessor, article_title_node
        )
        ...
        for section in body.findall(f"{prefix}div"):
            content, last_elem, refs, figures = self._parse_section_content(
                section, prefix
            )
            ...
            if head is not None and "n" in head.attrib:
                ...
                if len(content_graph) > 0:
                    if len(last_section_title) == 0:
                        dummy_node = self._add_node(
                            itg_doc,
                            "",
                            NTYPE_HEADING,
                            {"section": "1"},
                            predecessor=article_title_node,
                            parent=article_title_node,
                        )
                    ...
                    for c, l, r, f in content_graph:
                        sub_root, sub_last, node_map = self._add_subtree(
                            itg_doc, c, l, pred_parent, predecessor
                        )
                        ...
                        mapped_figs = {
                            fxid: node_map[fig.ix] for fxid, fig in f.items()
                        }
                ...
                section_title_node = self._add_node(
                    itg_doc,
                    section_name,
                    NTYPE_HEADING,
                    {"section": section_n},
                ...
                mapped_figs = {
                    fxid: node_map[fig.ix] for fxid, fig in current_figures.items()
                }
                body_figs.update(mapped_figs)
        ...
        for figure in body.findall(f"{prefix}figure"):
            fxid, figure_node = self._parse_figure(
                itg_doc, figure, prefix, predecessor, article_title_node
            )
            predecessor = figure_node
        ...
        bibliography = {}
        for bib_item in back.findall(
            f"{prefix}div/{prefix}listBibl/{prefix}biblStruct"
        ):
            xid, title, authors, pub_title, pub, pub_date = self._parse_bibitem(
                bib_item, prefix
            )

            bib_node = self._add_node(
                itg_doc,
                f"{', '.join(authors) if authors is not None else 'UNKNOWN'}, "
                f"{title}, "
        ...
        for node, refs in body_refs:
            for r in refs:
                ...
                # add span node
                ref_node = SpanNode(
                    ntype=NTYPE_BIB_REFERENCE
                    if rtype == "bibr"
                    else NYTPE_ELEMENT_REFERENCE,
                    src_node=node,
                    start=start,
                    end=end,
                ...
                if rtype == "bibr" and rtarget in bibliography:
                    ...
                    rtype == "figure" or rtype == "table"
                ) and target_node in body_figs:
    ...
    def _check_text(text: str, element_name: str) -> str:
        assert isinstance(
            text, str
        ), f"{element_name} is not a string, but a {type(text)}!"
        return text
    ...
    def _compare_section_counts(cntA: str, cntB: str) -> int:
        ...
        return (
            0 if len(nAs) == len(nBs) else -1
        )  # a higher level than b (a < b), else equal
...
class F1000XMLParserBM(F1000XMLParser):
    ...
    def __init__(self, xml_file_path: str):
        ...
        self.complex_ntype_mapping = [
            (
                F1000XMLParserBM._is_figure_ntype,
                F1000XMLParserBM._convert_to_figure_node,
            ),
            (F1000XMLParserBM._is_table_ntype, F1000XMLParserBM._convert_to_table_node),
            (
                F1000XMLParserBM._is_other_media_ntype,
                F1000XMLParserBM._convert_to_figure_node,
            ),
        ]
    ...
    def _is_figure_ntype(node):
        return (
            (
                node.ntype == "label"
                and (
                    node.content.lower().startswith("figure")
                    or node.content.lower().startswith("algorithm")
                    or node.content.lower().startswith("box")
                )
                or node.content.lower().startswith("listing")
            )
            or (node.ntype == "fig")
            or (node.ntype == "caption")
            or (node.ntype == "boxed-text")
        )
    ...
    def _parse_refs(self, ref_list: etree._Element) -> None:
        for ref in ref_list:
            ...
            title = (
                "".join([e.strip() for e in title.itertext()])
            ...
            if citation is not None:
                pub_info = [
                    i
                    for i in citation.findall("*")
                    if i.tag not in ["article-title", "person-group", "year"]
                ]
                pub_text = "".join(
                    [" ".join([t.strip() for t in i.itertext()]) for i in pub_info]
                )
            else:
                pub_text = None
            ...
            for a in authors:
                ...
                name_components += (
                    [given.text] if given is not None and given.text is not None else []
                )
                suffix = a.find("suffix")
                name_components += (
                    [suffix.text]
                    if suffix is not None and suffix.text is not None
                    else []
                )
            ...
            bib_item = Node(
                f"{', '.join(author_list) if len(author_list) > 0 else 'UNKNOWN'}, "
    ...
    def _make_node(
        ...
        if stringify:
            ...
            if content:
                return super(F1000XMLParser, self)._make_node(
                    content, element.tag, meta
                )
        else:
            content = ""
    ...
    def _split_element(
        cls, element: etree._Element, selector: str
    ) -> List[etree._Element]:
        ...
        assert (
            split_child_i != -1
        ), "something went wrong splitting an element. Invalid selector passed."
    ...
    ) -> Tuple[Optional[Node], List[etree._Element]]:
            ...
            if element.tag == "body":
                ...
                if label_element:
                    ...
                    node = self._make_node(
                        element.xpath("label")[0], stringify=True, meta=meta
                    )
                    ...
                    node = self._make_node(
                        element.xpath("caption")[0], stringify=True, meta=meta
                    )
                    # Remove the label tag and do another recursive call to parse element as an XML node
    ...
    def _parse_document(self):
        ...
        if doc is None:
            raise ValueError(
                f"The passed F1000 XML {self._xml_file_path} could not be parsed. Body not found."
            )

        # replace old node types by new ones, where an easy type mapping suffices
        nodes_by_ntype = [
            (newtype, [n for n in doc.nodes if n.ntype == oldtype])
            for oldtype, newtype in self.ntype_mapping.items()
        ]
...
class F1000XMLParserMetadata(F1000XMLParser):
    ...
    def _parse_all_metadata(self):
        ...
        self._meta["subjects"] = [
            s.text for s in subs[2:]
        ]  # first always = article type, not subject; second always = articles
        ...
        for review in self._root.xpath('.//sub-article[@article-type="ref-report"]'):
            ...
            license = review.find(".//license").attrib[
            ...
            contributors = [
                ...
                if contrib.tag == "contrib"
                and (contrib.find(".//name/surname") is not None)
            ]
            # todo get license
            meta = {
                "review_id": review_id,
    ...
    def _get_sentences_from_itg(self) -> List[SpanNode]:
        ...
        for node in self.itg.nodes:
            if node.ntype in [
                ...
                new_sentence_nodes = make_sentence_nodes(node, boundaries)
...
class IntertextLayoutTagger:
    ...
    def tag_text_with_position_information(self):
        ...
        out.meta.update({"position_tag_type": "from_draft"})
        ...
        for line in self.posinfo["lines"]:
            ...
            pix = next(
                i
                for i, p in enumerate(self.posinfo["pages"])
                if int(p[0][0]) <= int(li) <= int(p[1][0])
            )
                ...
                if node.ntype in [
                    ...
                    if lt_like in node.content.lower().strip():
                        ...
                        posi_node = SpanNode(
                            ...
                            meta={
                                "created_by": "IntertextLayoutTagger",
                                "line": li,
                                "page": pix + 1,
                            },
                        )
        ...
        for n, lpn in lines_per_node.items():
            pix0 = next(
                i
                for i, p in enumerate(self.posinfo["pages"])
                if int(p[0][0]) <= int(lpn[0]) <= int(p[1][0])
            )
            pix1 = next(
                i
                for i, p in enumerate(self.posinfo["pages"])
                if int(p[0][0]) <= int(lpn[1]) <= int(p[1][0])
            )
...
def f1000xml_to_itg(xml_path):
    assert os.path.exists(xml_path) and os.path.isfile(xml_path), (
        "provided file path to the XML does not exist. " "Cannot parse."
    )
...
def pdf_to_tei(pdf_path, config=None):
    assert os.path.exists(pdf_path) and os.path.isfile(pdf_path), (
        "provided file path to the PDF does not exist. " "Cannot parse."
    )
    ...
    _, status, parsed = client.process_pdf(
        "processFulltextDocument", pdf_path, **config
    )
