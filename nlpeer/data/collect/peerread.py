...
def load_peerread_papers(datapath):
    ...
    for s in ["train", "dev", "test"]:
        ...
        for p in list_files(parsed_path):
            ...
            papers[pid] = {
                "title": reviews_for_paper["title"],
                "abstract": reviews_for_paper["abstract"],
            }
...
def main(args):
    ...
    if args.download_approved:
        ...
        fetch_camera_ready_mapping(
            acl17_papers, "ACL2017", acl17_out_path, "external/acl-anthology/data"
        )

        conll16_papers = load_peerread_papers(conll16_path)
        fetch_camera_ready_mapping(
            conll16_papers, "CONLL16", conll16_out_path, "external/acl-anthology/data"
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script for matching PeerRead's ACL2017 and CONLL16 papers against"
