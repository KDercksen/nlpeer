...
from nlpeer.data.collect import fetch_camera_ready_mapping, retrieve_matched
...
def load_coling2020_papers(path):
    ...
    reviewed_paper_ids = [
        int(basename(f).split("_")[0]) for f in list_files(reviews_path)
    ]
    papers = {
        int(basename(f).split(".")[0]): f
        for f in list_files(papers_path)
        if int(basename(f).split(".")[0]) in reviewed_paper_ids
    }
    ...
    for pid, p_path in tqdm(papers.items(), desc="Iterating over papers"):
        ...
        title = root.find(
            f"{prefix}teiHeader/{prefix}fileDesc/{prefix}titleStmt/{prefix}title"
        ).text
        ...
        result[pid] = {"title": title, "abstract": abstract}
...
def main(args):
    ...
    if args.download_approved:
        ...
        if not os.path.exists(match_table):
            raise ValueError(
                f"The passed output directories do not contain an approved matching table. Make sure "
        ...
        fetch_camera_ready_mapping(
            papers, "COLING2020", out_path, "external/acl-anthology/data"
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script for matching PeerRead's COLING2020 papers against"
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        help="Path to the directory containing all datasets including a directory PeerRead "
    ...
    parser.add_argument(
        ...
        required=False,
        type=bool,
        help="Downloads the matched PDFs from the "
