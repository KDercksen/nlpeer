...
class SkimmingDataModule(LightningDataModule):
    ...
    def _load_splits_idx(self):
        fp = pjoin(
            self.base_path, self.dataset_type.value, "splits", "skimming_split.json"
        )

        assert os.path.exists(fp) and os.path.isfile(
            fp
        ), f"Cannot setup Actionability splits, as {fp} does not exist."
    ...
    def setup(self, stage: str | None) -> None:
        ...
        full_dataset = PaperReviewDataset(
            self.base_path,
            self.dataset_type,
            self.paper_version,
            self.paper_format,
        ...
        sk_dataset_test = SkimmingDataset(
            full_dataset,
            self.in_transform,
            self.target_transform,
            selected_types=["quote", "line", "sec-ix", "sec-name"],
        ...
        for split in self.dataset.keys():
            ...
            self.dataset[split] = self.dataset[split].map(
                self.convert_to_features, batched=False
            )
    ...
    def train_dataloader(self):
        ...
        return DataLoader(
            self.dataset["train"], **dl_config, collate_fn=self._collate_batches
        )
    ...
    def val_dataloader(self):
        ...
        return DataLoader(
            self.dataset["dev"], **dl_config, collate_fn=self._collate_batches
        )
    ...
    def test_dataloader(self):
        ...
        return DataLoader(
            self.dataset["test"], **dl_config, collate_fn=self._collate_batches
        )
...
def store_splits(dataset, splits, random_gen, out_path):
    jsplits = {
        ...
        "splits": [[[int(s), dataset.ids()[s]] for s in split] for split in splits],
...
def create_and_store_splits(
    full_data: SkimmingDataset, out_dir, splits: list[float], random_gen: int = None
):
    # covered papers
    ...
    splitted_pids_ix = paperwise_stratified_split(
        pids, splits, bucketed_samples_per_pid, random_gen
    )
    ...
    for spids in splitted_pids:
        ...
        for pid in spids:
            matching_samples = [
                (i, iid)
                for i, iid in enumerate(full_data.ids())
                if iid.split("%")[0] == pid
            ]
