...
def get_default_config():
    ...
    assert BENCHMARK_PATH is not None and os.path.exists(
        BENCHMARK_PATH
    ), f"Provided benchmark path is None or does not exist: {BENCHMARK_PATH}"

    return {
        "data_loader": {"num_workers": 0, "shuffle": False},
        "dataset": {"benchmark_path": BENCHMARK_PATH, "splits_from_file": True},
...
def classification_error(true_scores, predicted_scores, log=False):
    ...
    for sample_t, sample_p in zip(true_scores, predicted_scores):
        f1_accu += [
            sklearn.metrics.f1_score(
                sample_t, sample_p, pos_label=get_class_map_skimming()["positive"]
            )
        ]
        prec_accu += [
    ...
    if log:
        ...
        wandb.log(
            {
                "baseline": {
                    "f1": {"mean": np.mean(base_f1_accu), "std": np.std(base_f1_accu)},
                    "prec": {
                        "mean": np.mean(base_prec_accu),
                        "std": np.std(base_prec_accu),
                    },
                    "rec": {
                        "mean": np.mean(base_rec_accu),
                        "std": np.std(base_rec_accu),
                    },
                    "accuracy": {
                        "mean": np.mean(base_acc_accu),
                        "std": np.std(base_acc_accu),
                    },
                }
            }
        )
    ...
    return {
        ...
        "base_f1": {
            "mean": float(np.mean(base_f1_accu)),
            "std": float(np.std(base_f1_accu)),
        },
        "base_prec": {
            "mean": float(np.mean(base_prec_accu)),
            "std": float(np.std(base_prec_accu)),
        },
        "base_rec": {
            "mean": float(np.mean(base_rec_accu)),
            "std": float(np.std(base_rec_accu)),
        },
        "base_accuracy": {
            "mean": float(np.mean(base_acc_accu)),
            "std": float(np.std(base_acc_accu)),
        },
    }
...
def rank_measures(true_scores, estimated_scores, ks: list):
    ...
    for true_s, estimated_s in zip(true_scores, estimated_scores):
        ranking = list(
            sorted(zip(true_s, estimated_s), key=lambda x: x[1], reverse=True)
        )
        ...
        for k in ks:
            ...
            measures_at_k[f"precision@{k}"] = sklearn.metrics.precision_score(
                [r[0] for r in ranking[:k]],
                [get_class_map_skimming()["positive"] for r in ranking[:k]],
                pos_label=get_class_map_skimming()["positive"],
            )
            measures_at_k[f"recall@{k}"] = sklearn.metrics.recall_score(
                [r[0] for r in ranking[:k]],
                [get_class_map_skimming()["positive"] for r in ranking[:k]],
                pos_label=get_class_map_skimming()["positive"],
            )
            ...
            / (
                ...
                + float(
                    next(
                        ...
                        if r[0] == get_class_map_skimming()["positive"]
                    )
                )
            )
        ]
    ...
    result_at_k = {}
    mean_prec = {
        f"mean_precision@{k}": float(np.nanmean([m[f"precision@{k}"] for m in accu]))
        for k in ks
    }
    std_prec = {
        f"std_precision@{k}": float(np.nanstd([m[f"precision@{k}"] for m in accu]))
        for k in ks
    }
    ...
    mean_rec = {
        f"mean_recall@{k}": float(np.nanmean([m[f"recall@{k}"] for m in accu]))
        for k in ks
    }
    std_rec = {
        f"std_recall@{k}": float(np.nanstd([m[f"recall@{k}"] for m in accu]))
        for k in ks
    }
    ...
    result_mrr = {
        "mean_reciprocal_rank": float(np.mean(mrr)),
        "std_reciprocal_rank": float(np.std(mrr)),
    }
...
def ranking_error(true_relevance, estimated_relevance, log=False):
    precision_recall, auroc, aupr, mrr = rank_measures(
        true_relevance, estimated_relevance, ks=[2, 3, 4, 5, 6, 8]
    )

    random_baseline = [
        [float(x) for x in np.random.random(len(x)).tolist()] for x in true_relevance
    ]
    base_pr, base_auroc, base_aupr, base_mrr = rank_measures(
        true_relevance, random_baseline, ks=[2, 3, 4, 5, 6, 8]
    )
...
def setup(config):
    assert (
        "load_path" in config["model"]
    ), "Provide a 'load_path' attribute in the config.model to load a checkpoint"
    ...
    data_module_main = SkimmingDataModule(
...
def get_predictions(model, data_module, params, logger, debug=False):
    ...
    model.to(
        "cuda" if params["machine"]["device"] == "gpu" else params["machine"]["device"]
    )
...
def run(config, debug=False):
    ...
    with wandb.init(
        dir=os.path.join(OUT_PATH, "logs"),
        config=config,
        ...
        mconf_path = (
            pjoin(model_load_path, "config.json")
            if os.path.exists(pjoin(model_load_path, "config.json"))
            else None
        )
        checkpoints = [
            c
            for c in list_files(model_load_path)
            if os.path.basename(c) != "config.json"
        ]

        if len(checkpoints) == 0:
            print(
                f"WARNING: Model load path directory lacks checkpoints: {checkpoints}"
            )
        ...
        with open(
            pjoin(
                OUT_PATH,
                f"skimming_eval_{config['dataset']['type']}_{config['model']['type']}.json",
            ),
            "w+",
        ) as f:
        ...
        while len(measures) > 0:
            ...
            for key, val in items:
                if type(val) == dict:
                    ...
                    raise ValueError(
                        f"Not supported type {type(val)} found. Cannot aggregate."
                    )
...
def evaluation(checkpoints, config, model_config, wandb_logger, debug=False):
    ...
    for cpath in checkpoints:
        ...
        predictions = get_predictions(
            model, data_module_main, config, wandb_logger, debug
        )
        ...
        for batch in predictions:
            ...
            estimates += es
            pred_labels += [
                sample.detach().cpu().tolist() for sample in batch["predictions"]
            ]

            rankings += [
                [s[get_class_map_skimming()["positive"]] for s in e] for e in es
            ]
...
def main(args):
    ...
    assert os.path.exists(BENCHMARK_PATH) and os.path.exists(
        OUT_PATH
    ...
    config = {
        ...
        "evaluation": {"error": True, "ranking_error": True, "domain_ransfer": False},
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--project", required=True, type=str, help="Project name in WANDB"
    )
    parser.add_argument(
        "--store_results", required=True, type=str, help="Path for logs + results"
    )
    parser.add_argument(
        "--debug",
        required=False,
    ...
    parser.add_argument(
        "--chkp_dir", required=True, type=str, help="Path to load the checkpoints from"
    )
    parser.add_argument(
        "--dataset",
        required=True,
    ...
    parser.add_argument(
        "--model_type", required=True, type=str, help="Model type e.g. biobert"
    )
    parser.add_argument(
        "--paper_version",
        required=False,
