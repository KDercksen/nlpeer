...
class LitTransformerSkimmingModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(batch_size):
            out_p = self(
                attention_mask=positives[i]["attention_mask"],
                input_ids=positives[i]["input_ids"],
            ...
            out_n = self(
                attention_mask=negatives[i]["attention_mask"],
                input_ids=negatives[i]["input_ids"],
            ...
            tloss = self.train_loss(
                torch.cat((logits_p, logits_n), 0), torch.cat((targets_p, targets_n), 0)
            )
    ...
    def validation_step(self, batch, batch_idx) -> Optional[STEP_OUTPUT]:
        ...
        for i in range(batch_size):
            ...
            vloss = self.dev_loss(
                torch.cat((predictions_p, predictions_n), 0).flatten(),
    ...
    def predict_step(self, batch, batch_idx: int, dataloader_idx: int = None):
        ...
        for i in range(batch_size):
            ...
            predictions += [
                torch.concat((predictions_p.flatten(), predictions_n.flatten()), 0)
            ]
    ...
    def configure_optimizers(self):
        ...
        optimizer = self.optimizer(
            optimizer_grouped_parameters, lr=self.hparams.lr, eps=self.hparams.eps
        )
    ...
    def get_tokenizer(self) -> Callable:
        ...
        def tokenize(dataset):
            return tokenizer(
                dataset, padding="max_length", truncation=True, max_length=512
            )
...
def from_config(config):
    ...
    if (
        ...
        and "model" in config
        and "train" in config
        and "load_path" not in config["model"]
    ):
        ...
        return LitTransformerSkimmingModule(
            model_type=mtype,
            train_loss=get_loss_function(config["train"]["train_loss"]),
            dev_loss=get_loss_function(
                config["train"]["dev_loss"],
                **config["train"]["dev_loss_kwargs"]
                if "dev_loss_kwargs" in config["train"]
                else {}
            ),
    ...
    # from disk
        ...
        return LitTransformerSkimmingModule.load_from_checkpoint(
            config["model"]["load_path"]
        )
    else:
        raise ValueError(
            "Malformed config. Requires training regime and/or a model path to load from"
        )
