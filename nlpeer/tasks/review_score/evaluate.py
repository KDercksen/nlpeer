...
def relative_diversity(score_range, true_scores, predicted_scores, log=False):
    ...
    if log:
        t1 = wandb.Table(
            data=[
                [bucket, cnt]
                for bucket, cnt in zip(true_histogram[0], true_histogram[1])
            ],
            columns=["score", "cnt"],
        )
        t2 = wandb.Table(
            data=[
                [bucket, cnt]
                for bucket, cnt in zip(pred_histogram[0], pred_histogram[1])
            ],
            columns=["score", "cnt"],
        )
        ...
        t3 = wandb.Table(data=[[p] for p in predicted_scores], columns=["scores"])
        wandb.log(
            {
                f"diversity_fine_hist": wandb.plot.histogram(
                    t3, "scores", title="Predicted score dist"
                )
            }
        )
...
def relative_fairness(
    true_scores,
    predicted_scores,
    original_dataset,
    index_map,
    paper_criterion: Callable,
    criterion_name: str,
    log=False,
):
    crit_per_sample = np.array(
        [
            paper_criterion(original_dataset[index_map[i]])
    ...
    true_scores_per_crit = {
        c: np.array(true_scores)[samples_per_crit[c]] for c in crits
    }
    predicted_scores_per_crit = {
        c: np.array(predicted_scores)[samples_per_crit[c]] for c in crits
    }

    if len(crits) <= 1:
        print("Fairness not applicable -- only one class present!")
        wandb.log(
            {f"fairness_{criterion_name}": "not applicable, only one class present"}
        )
        return {}, {}
    ...
    h["human_annova"] = {
        "stat": float(annova.statistic),
        "pvalue": float(annova.pvalue),
    }
    ...
    if log:
        scores_per_crit = [
            ("true", true_scores_per_crit),
            ("pred", predicted_scores_per_crit),
        ]
        means = {}
        stds = {}
        ...
        fig.add_trace(
            go.Bar(
                x=list(crits),
                y=means["true"],
                error_y=dict(type="data", array=stds["true"]),
                name="True Scores",
            )
        )
        fig.add_trace(
            go.Bar(
                x=list(crits),
                y=means["pred"],
                error_y=dict(type="data", array=stds["pred"]),
                name="Pred Scores",
            )
        )
...
def setup(config):
    ...
    data_module = ReviewScorePredictionDataModule(
    ...
    original_data = ReviewPaperDataset(
...
def run(config, debug=False):
    ...
    with wandb.init(
        ...
        assert (
            len(checkpoints) > 0
        ), f"Model load path {model_load_path} directory lacks checkpoints: {checkpoints}"
        ...
        with open(
            pjoin(
                OUT_PATH,
                f"rsp_eval_{config['dataset']['type']}_{config['model']['type']}.json",
            ),
            "w+",
        ) as f:
...
def evaluation(checkpoints, config, model_config, wandb_logger, debug=False):
    ...
    for cpath in checkpoints:
        ...
        predictions = (
            torch.cat([x["predictions"] for x in predictions]).detach().cpu().tolist()
        )

        # assuming normalization of outputs
        predictions = [
            (i * (np.max(score_range) - np.min(score_range))) + np.min(score_range)
        ...
        if "diversity" in eval_conf and eval_conf["diversity"]:
            res["diversity"] = relative_diversity(
                score_range, labels, predictions, True
            )
        ...
        if "fairness" in eval_conf and eval_conf["fairness"] is not None:
            ...
            paper_criterion = get_criterion(
                criterion_name, original_dataset.dataset_type
            )
            res["fairness"] = relative_fairness(
                labels,
                predictions,
                original_dataset,
                imap,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--debug", required=False, default=False, type=bool, help="Turn on debugging"
    )
