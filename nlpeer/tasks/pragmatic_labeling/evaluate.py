...
def error(true_scores, predicted_scores, log=False):
    ...
    confusion_matrix = sklearn.metrics.confusion_matrix(
        true_scores, predicted_scores, labels=list(get_class_map_pragmatics().values())
    )

    majority_score = Counter(true_scores).most_common(1)[0][0]
    majority_baseline = [majority_score for s in true_scores]

    f1_micro_baseline = sklearn.metrics.f1_score(
        true_scores, majority_baseline, average="micro"
    )
    f1_macro_baseline = sklearn.metrics.f1_score(
        true_scores, majority_baseline, average="macro"
    )
        ...
        wandb.log(
            {
                "confusion_matrix": wandb.Table(
                    ...
                    columns=list(get_class_map_pragmatics().keys()),
                )
            }
        )
...
def setup(config):
    ...
    data_module_main = PragmaticLabelingDataModule(
...
def resetup(config, transforms):
    ...
    other = (
        DATASETS.F1000
        if config["dataset"]["type"] == DATASETS.ARR22.name
        else DATASETS.ARR22
    )
    data_module_secondary = PragmaticLabelingDataModule(
        benchmark_path=config["dataset"]["benchmark_path"],
        dataset_type=other,
        in_transform=input_transform,
        target_transform=target_transform,
...
def run(config, debug=False):
    ...
    with wandb.init(
        ...
        assert (
            len(checkpoints) > 0
        ), f"Model load path directory lacks checkpoints: {checkpoints}"
        ...
        with open(
            pjoin(
                OUT_PATH,
                f"stance_eval_{config['dataset']['type']}_{config['model']['type']}.json",
            ),
            "w+",
        ) as f:
    ...
    for cpath in checkpoints:
        ...
        if "domain_ransfer" in eval_conf and eval_conf["domain_ransfer"]:
            ...
            predictions2 = get_predictions(
                model, data_module_sec, config, wandb_logger, debug
            )
            labels2 = (
                torch.cat([x["labels"] for x in predictions2]).detach().cpu().tolist()
            )
            predictions2 = (
                torch.cat([x["predictions"] for x in predictions2])
                .detach()
                .cpu()
                .tolist()
            )
            ...
            res[
                f"transfer_from_{config['dataset']['type']}_to_{data_module_sec.dataset_type.name}"
            ] = error(labels2, predictions2, True)
...
def main(args):
    ...
    config = {
        ...
        "data_loader": {"batch_size": 15},
        "dataset": {
            "type": DATASETS[args.dataset].name,
            "paper_version": args.paper_version,
        },
        "evaluation": {"error": True, "domain_ransfer": True},
    }
