...
from nlpeer.tasks import (
    PragmaticLabelingDataset,
    random_split_pragmatic_labeling_dataset,
)
...
class PragmaticLabelingDataModule(LightningDataModule):
    ...
    def _load_splits_idx(self):
        ...
        assert os.path.exists(fp) and os.path.isfile(
            fp
        ), f"Cannot setup Pragmatic Labeling splits, as {fp} does not exist."
    ...
    def setup(self, stage: str | None) -> None:
        ...
        sd_dataset = PragmaticLabelingDataset(
            full_dataset, self.in_transform, self.target_transform
        )
        ...
        for split in self.dataset.keys():
            ...
            self.dataset[split] = self.dataset[split].map(
                self.convert_to_features, batched=True, remove_columns=["txt", "label"]
            )
...
def create_and_store_splits(
    full_data: PragmaticLabelingDataset,
    out_dir,
    splits: list[float],
    random_gen: int = None,
):
    split_ix, split_ids = random_split_pragmatic_labeling_dataset(
        full_data, splits, random_gen
    )
...
if __name__ == "__main__":
    ...
    assert set(DATASETS[d] for d in args.datasets).issubset(
        {DATASETS.ARR22, DATASETS.F1000}
